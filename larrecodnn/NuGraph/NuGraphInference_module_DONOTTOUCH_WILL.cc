  ...
  , eventDecoder(p.get<bool>("eventDecoder"))
{
  ...
  //
  if (eventDecoder) { produces<vector<FeatureVector<4>>>("event"); }
...
void NuGraphInference::produce(art::Event& e)
{
  ...
  std::unique_ptr<vector<FeatureVector<4>>> eventcol(new vector<FeatureVector<4>>(
    hitlist.size(), FeatureVector<4>(std::array<float, 4>({-1., -1., -1., -1.}))));
  ...
  if (eventDecoder) {
       ...
       {
    	   std::array<float, 4> evt_input({evt[i][0].item<float>(),
					   evt[i][1].item<float>(),
				           evt[i][2].item<float>(),
					   evt[i][3].item<float>()});
	   softmax(evt_input);
	   FeatureVector<4> evtt = FeatureVector<4>(evt_input); 
